...
export const LinkStakingSystem = () => {
  ...
  const { allowance, refetchAllowance } = useLINKAllowance(address || '' as `0x${string}`, STAKING_SYSTEM_CONTRACT_ADDRESS)
  const { data: userLinkInfo, refetchUserInfo } = useStakingUserLinkInfo(address || '' as `0x${string}`)
      ...
      {activeTab === 'stake' && (
        <div className="stake-form">
          ...
          <div className="staking-info">
            ...
            {/* 调试信息 */}
            {stakeAmount && (
              <div className="debug-info" style={{background: '#f0f8ff', border: '1px solid #ccc', padding: '10px', margin: '10px 0', fontSize: '12px', borderRadius: '4px'}}>
                <p><strong>🔍 调试信息:</strong></p>
                <p>质押数量: {stakeAmount} LINK ({parseUnits(stakeAmount, 18).toString()} wei)</p>
                <p>用户余额: {linkBalance ? formatUnits(linkBalance, 18) : '0'} LINK ({linkBalance?.toString()} wei)</p>
                <p>当前授权: {allowance ? formatUnits(allowance, 18) : '0'} LINK ({allowance?.toString()} wei)</p>
            ...
            {needsApproval && (
              <div className="approval-notice">
                <p><strong>⚠️ 注意：</strong>首次质押需要先授权LINK代币</p>
          ...
          {(approveError || stakingError) && (
            <div className="error">
              错误: {approveError?.message || stakingError?.message}
            </div>
          )}
          {stakingConfirmed && (
      ...
      {activeTab === 'withdraw' && (
        <div className="withdraw-form">
          ...
          <div className="withdraw-info">
            ...
            <p>• 系统会销毁对应的aLINK代币</p>
            <p>• 提取的LINK会发送到您的钱包</p>
            <p>• 最大可提取: {userLinkInfo ? formatUnits(userLinkInfo.stakedAmount, 18) : '0'} LINK</p>
          </div>
          ...
          {stakingError && (
            <div className="error">错误: {stakingError.message}</div>
          )}
      ...
      {activeTab === 'rewards' && (
        <div className="rewards-form">
          <div className="rewards-info">
            <h4>Aave LINK理财奖励</h4>
            <p>可领取奖励: {userLinkInfo ? formatUnits(userLinkInfo.availableRewards, 18) : '0'} LINK</p>
            <p>累计总奖励: {userLinkInfo ? formatUnits(userLinkInfo.totalRewardsClaimed, 18) : '0'} LINK</p>
            <p className="rewards-description">
              这些奖励来自您质押的LINK在Aave协议中的理财收益
            </p>
          </div>

          <button
            onClick={handleClaimRewards}
            disabled={!address || !userLinkInfo?.availableRewards || userLinkInfo.availableRewards === 0n || stakingPending || stakingConfirming}
            className="claim-btn"
          >
            {stakingPending
              ? '领取中...'

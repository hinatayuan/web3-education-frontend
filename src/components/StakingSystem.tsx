...
export const StakingSystem = () => {
  ...
  const { allowance, refetchAllowance } = useUSDTAllowance(address || '' as `0x${string}`, STAKING_SYSTEM_CONTRACT_ADDRESS)
          ...
          <div className="staking-info">
            ...
            {stakeAmount && (
              <div className="debug-info" style={{background: '#f0f8ff', border: '1px solid #ccc', padding: '10px', margin: '10px 0', fontSize: '12px', borderRadius: '4px'}}>
                ...
                <p>质押数量: {stakeAmount} USDT ({parseUnits(stakeAmount, 6).toString()} wei)</p>
                <p>用户余额: {usdtBalance ? formatUnits(usdtBalance, 6) : '0'} USDT ({usdtBalance?.toString()} wei)</p>
                <p>当前授权: {allowance ? formatUnits(allowance, 6) : '0'} USDT ({allowance?.toString()} wei)</p>
            ...
            {needsApproval && (
              <div className="approval-notice">
                <p><strong>⚠️ 注意：</strong>首次质押需要先授权USDT代币</p>
      ...
      {activeTab === 'rewards' && (
        <div className="rewards-form">
          <div className="rewards-info">
            ...
            <p>累计总奖励: {userInfo ? formatUnits(userInfo.totalRewardsClaimed, 6) : '0'} USDT</p>
          ...
          <button
            ...
            disabled={!address || !userInfo?.availableRewards || userInfo.availableRewards === 0n || stakingPending || stakingConfirming}
            className="claim-btn"
          >
            {stakingPending
              ? '领取中...'

...
import { useCourseManagerContract, useTokenBalance, useExchangeReserves, useContractBalances, useTokenAllowance, useYDTokenContract } from '../hooks/useContracts'
...
export const TokenExchange = () => {
  ...
  const { allowance, refetchAllowance } = useTokenAllowance(address || '', COURSE_MANAGER_CONTRACT_ADDRESS)
  const { buyTokens, sellTokens, mintTokenReserve, addETHReserve, isPending, isConfirming, isConfirmed, error } = useCourseManagerContract()
  ...
  const calculateEthAmount = (tokens: string) => {
    ...
    try {
      ...
      const ethAmount = tokenBigInt / EXCHANGE_RATE
  ...
  return (
    <div className="token-exchange">
      ...
      <div className="contract-status">
        ...
        {tokenReserve > 0n && sellTokenAmount && ethReserve > 0n && parseEther(sellTokenAmount) / 4000n > ethReserve && (
          <div className="warning">
      ...
      {activeTab === 'buy' && (
        <div className="exchange-form">
          ...
          {isConfirmed && activeTab === 'buy' && <div className="success">兑换成功！YD币已发放到您的钱包。</div>}
        </div>
      )}
        ...
        <div className="exchange-form">
          ...
          {needsApprovalForSell && (
            <div className="approval-notice">
              <p><strong>⚠️ 注意：</strong>出售YD币需要先授权合约</p>
          ...
          {(error || approveError) && <div className="error">错误: {error?.message || approveError?.message}</div>}
          {isConfirmed && activeTab === 'sell' && <div className="success">兑换成功！ETH已发送到您的钱包。</div>}
        </div>
      )}

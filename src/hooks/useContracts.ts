...
import { YDToken_CONTRACT_ADDRESS, COURSE_MANAGER_CONTRACT_ADDRESS, STAKING_SYSTEM_CONTRACT_ADDRESS, USDT_CONTRACT_ADDRESS, LINK_CONTRACT_ADDRESS, YDToken_ABI, COURSE_MANAGER_ABI, STAKING_SYSTEM_ABI, LINK_ABI } from '../config/contract'
...
export const useStakingUserInfo = (userAddress: `0x${string}`) => {
  ...
  const [stakedAmount, aTokenBalance, lastStakeTime, totalRewardsClaimed, availableRewards, currentValue] = userInfo
...
export const useStakingSystemStats = () => {
  ...
  const [_totalStaked, _totalRewardsPaid, _totalATokens, _availableRewards, _currentAPY] = systemStats
  
  return {
    totalStaked: _totalStaked as bigint,
    totalRewardsPaid: _totalRewardsPaid as bigint,
    totalATokens: _totalATokens as bigint,
    availableRewards: _availableRewards as bigint,
    currentAPY: _currentAPY as bigint
  }
}
...
export const useStakingLinkSystemStats = () => {
  ...
  const [_totalLinkStaked, _totalLinkRewardsPaid, _totalLinkATokens, _availableLinkRewards, _currentLinkAPY] = systemStats
  
  return {
    data: {
      totalLinkStaked: _totalLinkStaked as bigint,
      totalLinkRewardsPaid: _totalLinkRewardsPaid as bigint,
      totalLinkATokens: _totalLinkATokens as bigint,
      availableLinkRewards: _availableLinkRewards as bigint,
      currentLinkAPY: _currentLinkAPY as bigint
    },
